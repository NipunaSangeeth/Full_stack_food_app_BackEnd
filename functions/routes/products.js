...
router.post("/addToCart/:userId", async (req, res) => {
    ...
    const productId = req.body.productId; // get from frot end userID and ProductId(true the body tag)

    try {
        const doc = await db
            .collection("cartItems") // fetch the prodactiId
            .doc(`/${userId}/`)
            .collection("items")
            .doc(`/${productId}/`)
            .get();
        ...
        if (doc.data()) {
            const quantity = doc.data().quantity + 1; //update the quantity
            ...
            const addItems = await db // Push the to the cart item
                .collection("cartItems")
                .doc(`/${userId}/`)
                .collection("items")
                .doc(`/${productId}/`)
...
router.post("/updateCart/:user_id", async (req, res) => {
    ...
    try {
        ...
        if (doc.data()) {
            if (type === "increment") {
                const quantity = doc.data().quantity + 1; //update the quantity (cart)
                const updatedItem = await db
                    .collection("cartItems")
                    .doc(`/${userId}/`)
                    .collection("items")
...
// payment gateway part (using stripe )
...
//Stripe Webhook (Ensure the payment is done and get the more details)
let endpointSecret;
...
// Delete the cart item( Done paymet )
...
// Orders patr for Admin Dashboard
